...
class BloomFilterMightContainTest extends HyperspaceSuite {
  def test(values: Seq[Any], dataType: DataType): Unit = {
    ...
    values.foreach { v =>
      ...
      assert(BloomFilterMightContain(bfData, lit).eval() === bf.mightContain(v))
    }
  }

  test("BloomFilterMightContain works correctly for an int array.") {
    test((0 until 100000).map(_ * 2), IntegerType)
  }

  test("BloomFilterMightContain works correctly for a long array.") {
    test((0L until 100000L).map(_ * 2), LongType)
  }
