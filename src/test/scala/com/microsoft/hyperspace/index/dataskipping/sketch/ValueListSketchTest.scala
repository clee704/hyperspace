...
class ValueListSketchTest extends QueryTest with HyperspaceSuite {
  ...
  test("covertPredicate does not convert Not(EqualTo(<col>, <lit>)).") {
    ...
    val expected = Some(
      And(
        Not(EqualTo(Size(sketchValues(0)), Literal(0))),
        Or(
          Not(EqualTo(Size(sketchValues(0)), Literal(1))),
  ...
  test("covertPredicate converts In.") {
    ...
    val result = sketch.convertPredicate(
      ...
      exprIdColMap,
      Seq(AttributeReference("A", IntegerType)(ExpressionUtils.nullExprId)))
    val expected = Some(SortedArrayIntersects(sketchValues(0), Literal.create(Seq(23, 42))))
    assert(result === expected)
  }
  ...
  test("covertPredicate converts In - string type.") {
    ...
    val expected =
      Some(
        SortedArrayIntersects(
          sketchValues(0),
          Literal.create(Seq("hello", "world"), ArrayType(StringType, containsNull = false))))
    assert(result === expected)
  }
}
