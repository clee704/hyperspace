...
object ExpressionUtils {
  ...
  object ExtractKnownNotNullArgs {
  ...
  def getResolvedExprs(
    ...
    val resolvedExprs = sketches.map { s =>
      val parsedExprs = s.expressions.map {
        case (expr, _) => DummyPredicate(spark.sessionState.sqlParser.parseExpression(expr))
      }
      ...
      val resolved = filter.condition.collect {
        case DummyPredicate(expr) => normalize(expr)
      }
  ...
  case class DummyPredicate(override val child: Expression)
